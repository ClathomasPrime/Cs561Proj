...
\section{Results}
  ...
  \begin{proof}
    ...
    by $m$. Specifically, suppose $P$ contains a subpath $uR$, where $R$ is not
    actually used by $u$, and suppose a node $q$ adjacent to $u$ installs a path
    containing $P$ (i.e. $q$ routes through $m$ based on $m$'s lie),
    so that $q$ can announce its path to $u$ and loop verification will detect
    the lie. ((ANOTHER TOO BIG SENTENCE PLS HELP))

...
\section{Model Details}
  ...
  \subsection{Verification}
    \todo{
      Not honestly sure what to do with this subsection.
      It's mostly just ``previous work''
    }
    As mentioned before, various methods of catching
    lies have been suggested in the literature.
    ...
    \begin{definition} 
      ...
      specifically because that neighbor is already in the path. In addition, if
  ...
  \subsection{Behavioral assumptions}
    ((DEFINE NEXT-HOP POLICY, VOLUME ATTRACTION, AND GENERIC
    ATTRACTION))
...
\section{Results/examples}
  ...
  \begin{theorem}
    ...
    and all other nodes honestly participate in BGP and in next-hop verification.
    Then $m$ will be caught by next-hop verification if and only if
    $m$ would be caught if loop verification were used by all other nodes.
    ((TODO: maybe phrase this better))
  \end{theorem}
  \begin{proof}
    TODO: this
  \end{proof}
  As an aside, we note that an analogue of the above theorem for path
  verification does not hold ((DISCUSS BOWTIE MINUS AN EDGE)).

...
\section{Next-Hop Verification Protocol}
  ...
  ((TODO: DESCRIBE THE STUFF I COMMENTED OUT IN WORDS))
  ...
  The following additional considerations need to be considered:
  ((PHRASING?))
  \begin{itemize}
...
\section{Results}
  ...
  \begin{theorem}
    ...
    and all other nodes honestly participate in BGP and in next-hop verification.
    Then if $m$ would be necessarily be caught if loop verification
  ...
  \begin{proof}
    ...
    $P$ from $m$ to $d$. Consider all the AS's that this announcement travels
    ...
    node $q$ along the path $Q$ is adjacent to a node $u$, such that $P$ contains
    the path $uR$, where $R$ is a route that $u$ did not announce. Indeed,
    in this case a route containing $P$ can be announced to $u$, and $u$ can
    raise the alarm for loop verification.

    Let $(a,b)$ be the first hop along $R$ which is not actually used in the
    data plane.
    If next-hop verification is used by all non-manipulator nodes, then $q$ will
    ask $u$ the query $Q_d(a,b)$. This query will travel along the path $R$,
    until it gets to $a$, and the alarm will be raised, as in the main lemma.
  \end{proof}
...
\section{Possible future directions}
  ...
  \todo{Several of these next paragraphs are good but seem too long to me.
  I can trim them down if that's cool!}
  ...
  Intuitively, it seems like partial deployment and participation would still be
  at least somewhat valuable. While this would not give us the same degree of
  ...
  ((MENTION THAT TRUE FLOODING IS OBVIOUSLY IMPRACTICAL AND AT VERY LEAST YOU
  NEED A TIME TO LIVE THING. MORE GENERALLY WE'RE JUST TRYING TO DEMONSTRATE THE
  EFFECTIVENESS OF THIS IDEA.))

  We see a possible value in using this sort of approach, but we also see
  benefits in the unencrypted ``flooding'' approach. The biggest problem we see
